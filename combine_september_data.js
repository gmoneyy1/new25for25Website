...
/**
 * Main function to combine both September datasets and add distances
 */
function combineSeptemberData() {
  try {
    console.log('🔄 Starting September data combination...');
    
    // Read both September data files
    const sept1_15File = 'sept_data.csv';
    const sept16_30File = 'data/all_sept16_30_successful.csv';
    const outputFile = 'sept_data_complete_with_distances.csv';
    
    if (!fs.existsSync(sept1_15File)) {
      console.error(`❌ File ${sept1_15File} not found!`);
      return;
    }
    
    if (!fs.existsSync(sept16_30File)) {
      console.error(`❌ File ${sept16_30File} not found!`);
      return;
    }
    
    const sept1_15Content = fs.readFileSync(sept1_15File, 'utf8');
    const sept16_30Content = fs.readFileSync(sept16_30File, 'utf8');
    
    const sept1_15Lines = sept1_15Content.trim().split('\n');
    const sept16_30Lines = sept16_30Content.trim().split('\n');
    
    console.log(`📊 September 1-15: ${sept1_15Lines.length} lines`);
    console.log(`📊 September 16-30: ${sept16_30Lines.length} lines`);
    ...
    // Process data rows
    const newLines = [unifiedHeader.join(',')];
    let processedCount = 0;
    let errorCount = 0;
    
    // Process September 1-15 data (already in correct format)
    console.log('🔄 Processing September 1-15 data...');
    for (let i = 1; i < sept1_15Lines.length; i++) {
      try {
        const line = sept1_15Lines[i].trim();
        if (!line) continue;
        
        const values = parseCsvLine(line);
        
        // Based on actual data structure from before:
        const flightNumber = values[0] || '';     // First column is Flight Number
        const origin = values[1] || '';           // Second column is Origin
        const destination = values[2] || '';      // Third column is Destination
        const departureDate = values[3] || '';    // Fourth column is Departure Date
        const arrivalTime = values[4] || '';      // Fifth column is Arrival Time
        const duration = values[5] || '';         // Sixth column is Duration
        const price = values[6] || '';            // Seventh column is Price
        const stops = values[7] || '';            // Eighth column is URL (we'll use as Stops)
        ...
        if (i % 1000 === 0) {
          console.log(`✅ Processed ${i} lines from Sept 1-15...`);
        }
        
      } catch (error) {
        console.warn(`⚠️ Error processing Sept 1-15 line ${i + 1}:`, error.message);
        errorCount++;
      }
    }
    
    // Process September 16-30 data
    console.log('🔄 Processing September 16-30 data...');
    for (let i = 1; i < sept16_30Lines.length; i++) {
      try {
        const line = sept16_30Lines[i].trim();
        if (!line) continue;
        
        const values = parseCsvLine(line);
        
        // Based on the structure I saw:
        const flightNumber = values[0] || '';     // Flight Number
        const origin = values[1] || '';           // Origin
        const destination = values[2] || '';      // Destination
        const departureDatetime = values[3] || ''; // Departure Datetime
        const arrivalDatetime = values[4] || '';   // Arrival Datetime
        const duration = values[5] || '';         // Elapsed Minutes
        const price = values[6] || '';            // Cheapest Price
        const stops = values[7] || '';            // Search URL (we'll use as Stops)
        ...
        if (departureDatetime) {
          const depParts = departureDatetime.split(' ');
          departureDate = depParts[0] || '';
          arrivalTime = arrivalDatetime ? arrivalDatetime.split(' ')[1] || '' : '';
        }
        ...
        if (i % 1000 === 0) {
          console.log(`✅ Processed ${i} lines from Sept 16-30...`);
        }
        
      } catch (error) {
        console.warn(`⚠️ Error processing Sept 16-30 line ${i + 1}:`, error.message);
        errorCount++;
      }
    }
    ...
    console.log('\n🎉 September data combination completed!');
    console.log(`📊 Total lines processed: ${processedCount}`);
    console.log(`❌ Errors encountered: ${errorCount}`);
    console.log(`📁 Output file: ${outputFile}`);
    console.log(`📏 Structure: ${unifiedHeader.join(' → ')}`);
