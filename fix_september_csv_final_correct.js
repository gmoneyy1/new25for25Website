...
/**
 * Main function to fix the September CSV with correct column order
 */
function fixSeptemberCsvFinalCorrect() {
  try {
    console.log('🔄 Starting September CSV final correct fix...');
    
    // Read the input CSV file
    const inputFile = 'september_1_15_all_successful.csv';
    const outputFile = 'september_1_15_perfect.csv';
    ...
    // Based on the actual data structure I observed:
    // The data rows have: Flight Number, Origin, Destination, Departure Date, Arrival Time, Duration, Price, URL, Route Type, Empty
    // But the header says: Origin, Destination, Departure Date, Flight Number, Departure Time, Arrival Time, Duration, Price, Stops, Route Type
    
    // Create new header with Flight Number FIRST
    const newHeader = [
      'Flight Number',
      'Origin', 
      'Destination',
    ...
    for (let i = 1; i < lines.length; i++) {
      try {
        ...
        // values[8] is Route Type - we skip this
        // values[9] is empty - we skip this
        ...
        // Create new row with Flight Number FIRST
        const newRow = [
          flightNumber,           // Flight Number first
          origin,                 // Origin
          ...
          duration,               // Duration
          price,                  // Price
          stops,                  // Stops (using URL field)
          distance.toString()     // Distance (MI)
        ];
    ...
    console.log('\n🎉 September CSV final correct fix completed!');
...
// Run the final correct fix
fixSeptemberCsvFinalCorrect();
