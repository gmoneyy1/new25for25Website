 ...
 */
function fixSeptemberCsvCorrect() {
  try {
    console.log('🔄 Starting September CSV correct fix...');
    
    // Read the input CSV file
    const inputFile = 'september_1_15_all_successful.csv';
    const outputFile = 'september_1_15_correct.csv';
    ...
    // 0: Origin, 1: Destination, 2: Departure Date, 3: Flight Number, 4: Departure Time, 5: Arrival Time, 6: Duration, 7: Price, 8: Stops, 9: Route Type
    ...
    for (let i = 1; i < lines.length; i++) {
      try {
        ...
        const origin = values[0] || '';           // Origin
        const destination = values[1] || '';      // Destination
        const departureDate = values[2] || '';    // Departure Date
        const flightNumber = values[3] || '';     // Flight Number
        ...
        const stops = values[8] || '';            // Stops
        // values[9] is Route Type - we skip this
    ...
    console.log('\n🎉 September CSV correct fix completed!');
